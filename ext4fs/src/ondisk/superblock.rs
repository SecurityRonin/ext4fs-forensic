...
impl Superblock {
    ...
    pub fn parse(buf: &[u8]) -> Result<Self> {
        ...
        let mkfs_time = if buf.len() > 0x10C {
            le32(buf, 0x108)
        } else {
            0
        };

        let log_groups_per_flex = if buf.len() > 0x16C {
            le32(buf, 0x168)
        } else {
            0
        };
        ...
        let first_error_time = if buf.len() > 0x194 {
            le32(buf, 0x190)
        } else {
            0
        };
        let last_error_time = if buf.len() > 0x1B4 {
            le32(buf, 0x1B0)
        } else {
            0
        };

        let checksum_seed = if buf.len() > 0x260 {
            le32(buf, 0x25C)
        } else {
            0
        };
        let checksum = if buf.len() >= 0x400 {
            le32(buf, 0x3FC)
        } else {
            0
        };
    ...
    pub fn group_count(&self) -> u32 {
        ...
        let per = u64::from(self.blocks_per_group);
...
mod tests {
    ...
    fn reject_invalid_magic() {
        ...
        assert!(matches!(
            err,
            crate::error::Ext4Error::InvalidMagic { found: 0 }
        ));
    }
    ...
    fn uuid_string_format() {
        ...
        let sb = Superblock::parse(&buf).unwrap();
        assert_eq!(sb.uuid_string(), "01234567-89ab-cdef-fedc-ba9876543210");
    }
